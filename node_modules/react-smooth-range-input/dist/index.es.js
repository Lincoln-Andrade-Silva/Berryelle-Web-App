...
var SliderIndicator = ({ amount, color, isThin, barColor, hasTickMarks }) => (createElement("div", { style: Object.assign({ display: 'flex', width: '100%', justifyContent: 'space-around', minHeight: '10px', pointerEvents: 'none' }, (isThin ? { background: barColor, borderRadius: '4px' } : { paddingTop: '15px' })) }, hasTickMarks &&
    createArrayWithNumbers(amount > 50 ? 50 : amount).map(index => (createElement("span", { style: {
            height: '10px',
            width: '1px',
            background: color,
        }, key: index })))));
...
var Controller = forwardRef(function Controller({ onFocus, onBlur, controllerHeight = 0, controllerWidth = 0, height, dragX, showBubble, isControlByKeyBoard, value, onMouseDown, onInteractEnd, barColor, textBackgroundColor, textColor, isThin, isTouchDevice, disabled, shouldDisplayValue, customController, max, min, shouldAnimateNumber, isFocusing, focusStyle, }, ref) {
    ...
    return (createElement("div", Object.assign({}, {
        ...
        !customController && (createElement(Animate, Object.assign({ play: showBubble }, commonAnimationProps, { duration: showBubble ? 0.2 : 0.3, start: {
                transform: 'translateY(0)',
            }, end: {
                transform: `translateY(-${isThin ? 32 : 22}px) scale(1.65)`,
            }, easeType: "cubic-bezier(0.86, 0, 0.07, 1)", render: ({ style }) => (createElement("svg", { style: Object.assign({ position: 'absolute', top: '-25px', left: 0 }, style), x: "0px", y: "0px", width: `${controllerHeight}px`, height: "64px", viewBox: "0 0 40 64" },
                createElement("path", { style: {
                        transition: '0.3s all',
                        fill: barColor,
                    }, d: showBubble ? bubbleWithTail : bubble }))) }))),
        createElement(Animate, Object.assign({ play: showBubble }, commonAnimationProps, { end: {
                transform: `translateY(-${isThin ? 58 : 48}px) scale(1.3)`,
            }, duration: showBubble ? 0.3 : 0.1, render: ({ style }) => customController ? (customController({
                ref: controllerRef,
                value,
            })) : (createElement("div", { style: Object.assign({ background: textBackgroundColor, height: `${controllerHeight}px`, width: `${controllerHeight}px`, borderRadius: '50%', position: 'absolute' }, (isFocusing && !isTouchDevice ? { boxShadow: focusStyle || 'rgb(0, 0, 0) 0px 0px 6px' } : null), style), "aria-hidden": "true" }, shouldDisplayValue && (createElement("span", { style: Object.assign({ position: 'absolute', left: 0, width: `${controllerHeight}px`, color: textColor, fontWeight: 600 }, (shouldAnimateNumber
                    ? { top: '10px' }
                    : {
                        top: 0,
                        display: 'flex',
                    ...
                    })) }, shouldAnimateNumber ? createElement(FlipNumbers, Object.assign({}, flipNumberProps, { numbers: value.toString() })) : value)))) }))));
});
...
class Slider extends PureComponent {
    constructor() {
        ...
        this.controllerRootRef = createRef();
        this.touchDevice = isTouchDevice();
        this.controllerRef = createRef();
